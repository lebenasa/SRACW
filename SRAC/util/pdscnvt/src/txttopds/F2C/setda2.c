/* setda2.f -- translated by f2c (version 20100827).
...
/* ----------------------------------------------------------------------- */
/*     SETDA2 : SET DATA FROM TEXT LINE(A72) INCLUDING INTEGER OR */
/*              FLOATING NUMBER OR CHRACTER DATA IN MEMBER ----BNUP */
/*              SAMPLE: */
/*               1.00000E+02 CASE               125 -3.00000E-12 ....... */
/* ----------------------------------------------------------------------- */
/* Subroutine */ int setda2_(char *line, integer *ld, real *data, integer *
...
L110:
	...
	iis = setdt_1.nzon2 * 3 + 31;
	iie = iis + setdt_1.ntnuc2 - 1;
	if (lpos >= iis && lpos <= iie) {
	    s_copy(cwork + (i__ - 1 << 2), adata + ((i__ - 1) * 12 + 1), (
		    ftnlen)4, (ftnlen)4);
	    goto L100;
...
/*         WRITE(6,*) I,WORK(I) */
	    ...
	    if (lpos == 2) {
		setdt_1.ntnuc2 = iwork[i__ - 1];
	    }
	    if (lpos == 3) {
		setdt_1.nzon2 = iwork[i__ - 1];
	    }
	}
L100:
	;
